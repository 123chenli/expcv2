...
imgs = glob.glob("../calibration/calibrateImages/*.jpg")
for fname in imgs:
...
# undistort方法二
print('--------------使用重映射的方式-----------------')
mapx, mapy = cv2.initUndistortRectifyMap(mtx, dist, None, newcameramtx, (w, h), 5)  # 获取映射的方程
dst = cv2.remap(img, mapx, mapy, cv2.INTER_CUBIC)  # 重映射后，图像变小了
x, y, w, h = roi
dst2 = dst[y: y+h, x: x+w]
cv2.imwrite('calibratesult11_2.jpg', dst2)
print('方法二： dst的大小为：', dst2.shape)  # 图像比方法一小
